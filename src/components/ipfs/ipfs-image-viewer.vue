...
#avatar-container(v-if="ipfsCid || showDefault")
  q-avatar(:size="size" :color="color" :text-color="textColor" v-if="!square")
    img.object-cover(:src="imageURI" v-if="imageURI")
    loading-spinner.loadingSpinner(:color="textColor" v-else-if="!imageURI && isLoading")
    span(v-else-if="!imageURI && !isLoading && showDefault" color="primary" text-color="white") {{ this.label }}
  template(v-else)
    q-card(:style="{ 'max-width': '180px', 'border-radius': '12px', 'box-shadow': '0px 0px 14px #23283C14', 'position': 'relative' }")
      .bg-white.flex.items-center.justify-center.absolute.cursor-pointer(v-if="canRemoveFile" @click="$emit('removeFile')" :style="{ 'width': '20px', 'height': '20px', 'border-radius': '50%', 'right': '10px', 'top': '10px' }")
        q-icon(name="fas fa-times" color="primary")
      img.object-cover(:src="imageURI" v-if="imageURI" width="180px" height="98px" :style="{ 'border-radius': '12px 12px 0 0' }")
      q-card-section(v-if="originalUploadedFile")
        ...
        .h-b2.text-italic {{ Math.round(originalUploadedFile.size/1000) }}
          | KB

...
widget.full-height.full-width(:title="singleCircle ? '' : title" :noPadding="singleCircle")
  template(v-if="!singleCircle")
    ...
    q-tabs(active-color="primary" align="start" indicator-color="primary" no-caps="no-caps" v-model="tab" v-if="circles.length > 0")
      ...
      template(v-for="circle in circles")
        widget.bg-internal-bg.q-my-md.cursor-pointer(:style="{ 'padding-right': '10px' }")
          div(:style="{ 'padding-right': '20px' }")
            ...
            .row.no-wrap
              q-linear-progress.q-mt-xs(rounded="rounded" size="8px" color="primary" :value="percentage(circle.husd[0] ? circle.husd[0] : 0,totalBudget)/100")
              .text-italic.text-xs.q-mt-xxs.q-ml-md {{ formatCurrency(circle.husd[0] ? circle.husd[0] : 0) }}
            .row.no-wrap
              q-linear-progress.q-mt-xs(rounded="rounded" size="8px" color="secondary" :value="percentage(circle.hvoice[0] ? circle.hvoice[0] : 0,totalBudget)/100")
            ...
            .template.col-6.q-pr-md.q-pb-md(v-for="subCircle in circle.subcircles" :style="{ 'padding-right': '20px' }" :class=" { 'full-width': $q.screen.lt.md }")
              widget
                .row.items-center
                  .h-h6 {{ subCircle.name }}
                .row.no-wrap
                  q-linear-progress.q-mt-xs(rounded="rounded" size="8px" color="primary" :value="percentage(subCircle.husd[0] ? subCircle.husd[0] : 0,circle.husd[0] ? circle.husd[0] : 0)/100")
                  .text-italic.text-xs.q-mt-xxs.q-ml-md {{ formatCurrency(subCircle.husd[0] ? subCircle.husd[0] : 0) }}
                .row.no-wrap
                  q-linear-progress.q-mt-xs(rounded="rounded" size="8px" color="secondary" :value="percentage(subCircle.hvoice[0] ? subCircle.hvoice[0] : 0,circle.hvoice[0] ? circle.hvoice[0] : 0)/100")

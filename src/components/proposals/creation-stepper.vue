<script>
/**
 * Control for navigating the proposal creation wizard
 */
export default {
  name: 'creation-stepper',
  components: {
    Widget: () => import('~/components/common/widget.vue')
  },

  props: {
    /**
     * The current active step
     */
    stepIndex: Number,
    /**
     * An array of objects containing the steps
     * including a label and key
     */
    steps: Array
  }
}
</script>

<template lang="pug">
widget(title="Creation process")
  q-list
    template(v-for="(s, i) in steps")
      q-item(clickable v-ripple :key="s.key" @click="$emit('goto', s.key)")
        q-item-section(avatar)
          // q-avatar(color="white" text-color="grey-5") {{ i + 1 }}
          q-btn(v-if="stepIndex === i" round unelevated color="primary") {{ i + 1 }}
          q-btn(v-else outline round unelevated color="grey-5" :ripple="false") {{ i + 1 }}
        q-item-section.text-subtitle1(:class="{ 'text-bold': stepIndex === i }") {{ s.label }}
  q-btn.q-mt-lg.q-px-sm.full-width(rounded outline color="primary" no-caps) Save as draft
  q-btn.q-my-sm.q-px-sm(rounded unelevated color="primary" no-caps).full-width Publish to staging
</template>

...
.proposal-draft.justify-between.items-center(:class="{ 'row':$q.platform.is.desktop }")
  ...
  .q-pa-sm(:class="{ 'col-3':$q.platform.is.desktop }")
    q-btn.full-width(color="primary" rounded no-caps @click="$emit('continue')") {{ $t('proposals.proposal-draft.continueProposal') }}
    q-btn.q-mt-sm.full-width(flat rounded no-caps @click="$emit('delete')") {{ $t('proposals.proposal-draft.deleteDraft') }}

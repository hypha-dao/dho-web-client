  ...
  .row
    ...
    .col.q-pl-md
      ...
      span.relative-position
        q-btn.q-px-xl.rounded-border.text-bold.q-mt-xl.relative-position(:disable="!isAdmin" @click="isEditing = true" color="white" :label="$t('ecosystem.ecosystem-info.editEcosystemInformation')" no-caps="no-caps" outline="outline" rounded="rounded" unelevated="unelevated" v-if="!isEditing")
      ...
      q-btn.q-px-xl.rounded-border.text-bold.q-mt-xl(:disable="!isAdmin" @click="_handleSave" color="secondary" :label="$t('ecosystem.ecosystem-info.saveEcosystemInformation')" no-caps="no-caps" rounded="rounded" unelevated="unelevated" v-if="isEditing")
  section(v-if="isEditing")
    .row.q-col-gutter-md.justify-between.q-pt-xl.q-px-md
      .col-4
        ...
        q-input.q-my-sm(:debounce="200" :rules="[rules.required]" bg-color="#343B64" color="white" dark="dark" dense="dense" lazy-rules="lazy-rules" outlined="outlined" :placeholder="$t('ecosystem.ecosystem-info.typeNameHere')" ref="name" rounded="rounded" v-model="form.name")
      ...
      .col-4
        .row.items-center
          ...
          .q-pl-xs
            ...
            q-btn.q-my-sm.q-px-xl.rounded-border.text-bold.full-width(@click="$refs.ipfsInput.chooseFile()" color="white" :label="$t('ecosystem.ecosystem-info.uploadAnImage')" no-caps="no-caps" outline="outline" rounded="rounded" unelevated="unelevated")
    ...
    .row.q-mt-xl
      ...
      .row.q-col-gutter-md.full-width
        template(v-for="(domain,index) in ECOSYSTEM_DOMAIN")
          .col-3
            q-btn.radio-button.q-my-sm.q-px-xl.rounded-border.text-bold.full-width(@click="form.domain = domain" color="white" :label="parseEcosystemDomain(domain)" no-caps="no-caps" outline="outline" rounded="rounded" unelevated="unelevated")
              q-radio(dense="dense" dark="dark" v-model="form.domain" :val="domain" color="white")
    .row.q-px-md.q-mt-xl
      .full-width
        label.h-label.text-white {{ $t('ecosystem.ecosystem-info.ecosystemPurpose') }}
        q-input.q-my-sm.rounded-border(:debounce="200" :input-style="{ 'resize': 'none' }" :rules="[rules.required]" bg-color="#343B64" color="white" dark="dark" dense="dense" lazy-rules="lazy-rules" maxlength="300" outlined="outlined" :placeholder="$t('ecosystem.ecosystem-info.typeADescription')" ref="purpose" rounded="rounded" rows="5" type="textarea" v-model="form.purpose")

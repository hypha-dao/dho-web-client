...
.period-card.q-ml-none.q-mr-xxs
  q-btn.q-ma-none(v-if="mini" :color="backgroundColor" flat :round="moon" :icon="icon" :icon-only="moon" :hide-icon="!moon" clickable size="xs")
    q-tooltip(anchor="top middle" self="bottom middle" :content-style="{ 'font-size': '1em', width: '142px' }")
      div(v-html="miniText")
  q-btn.expanded-card(v-else :class="{ 'no-pointer-events': !clickable }" :color="backgroundColor" :text-color="textColor" :outline="end > now" :disable="start > now" unelevated no-caps @click="clickable && $emit('click')")

...
export default {
  ...
  computed: {
    ...
    isCreateDao() {
      return this.$router.currentRoute.name === 'create-your-dao'
    },
    isLoggedIn() {
  ...
  methods: {
    goNext() {
      if (this.isCreateDao && localStorage.getItem('autoLogin')) {
...
.full-width.full-height.flex(:class="{ 'items-start q-pa-xl':!$q.screen.gt.md, 'items-center':$q.screen.gt.md }")
  .full-width.full-height.flex.column.justify-between.no-wrap(:style="{ 'padding-bottom': '50px' }")
    .full-height.flex.column(:class="{ 'justify-start': !$q.screen.gt.md, 'justify-center': $q.screen.gt.md }")
      .row(v-if="isCreateDao" :class="{ 'q-mb-xxxl': $q.screen.gt.md }")
      ...
      .h-b1-signup.text-h-gray.q-py-xxs.leading-loose(v-if="$q.screen.gt.md && !isOnboarding")
        template(v-if="isCreateDao") {{ $t('login.welcome-view.ifYouAlreadyHaveAHyphaAccountPangea') }}
    ...
    div
    q-btn.full-width.q-mt-xl.text-bold(v-if="!isLoggedIn" :disable="false" @click="$emit('onRegisterClick')" color="primary" no-caps rounded unelevated)
    ...
    q-btn.full-width.q-mt-sm.text-bold(@click="goNext()" color="secondary" no-caps rounded unelevated)
      .full-width.flex.justify-between
        q-avatar(size="20px")
          img(src="/chains/pangea.png")
        div(v-if="isLoggedIn") {{ $t('login.welcome-view.createDao') }}
        div(v-else) {{ $t('login.welcome-view.login') }}
        div
      </q-avatar>
    q-btn.full-width.q-mt-sm.text-bold(:to="{ name:'dashboard' }" color="primary" :label="$t('login.welcome-view.continueAsAGuest')" no-caps outline rounded unelevated v-if="!isOnboarding && !isCreateDao")

...
q-card.widget.full-width.q-pt-xl.q-pl-xl.q-pr-xs.q-pb-xs.relative-position.rounded(:class="{'q-pr-xl': $q.screen.md || $q.screen.lt.md}" flat="flat")
  .col
    .row.justify-between.items-center
      .row.items-center
        img(height="14px" src="/svg/check-to-slot.svg" width="18px")
        .title.text-bold.q-ml-sm {{ $t('common.upvote-delegate-widget.upvoteDelegates') }}
      .row(:class="{'q-mt-md': $q.screen.lt.md}")
        .counter(:class="{'right-margin': !$q.screen.lt.md}")
          .title {{ $t('common.upvote-delegate-widget.electionValidityExpiresIn') }}
          .time.row
            .row.items-end
              .days {{formatTimeLeft().days}}
              .subtext(v-if="formatTimeLeft().days > 1") {{ $t('common.upvote-delegate-widget.days') }}
              .subtext(v-else) {{ $t('common.upvote-delegate-widget.day') }}
            .row.items-end
              .hours {{formatTimeLeft().hours}}
              .subtext(v-if="formatTimeLeft().hours > 1") {{ $t('common.upvote-delegate-widget.hours') }}
              .subtext(v-else) {{ $t('common.upvote-delegate-widget.hour') }}
            .row.items-end
              .mins {{formatTimeLeft().mins}}
              .subtext(v-if="formatTimeLeft().mins > 1") {{ $t('common.upvote-delegate-widget.mins') }}
              .subtext(v-else) {{ $t('common.upvote-delegate-widget.min') }}
    .row.q-mt-md
      .template.col(:class="{'col-6 q-px-xs': $q.screen.md, 'q-mr-md q-mb-md': $q.screen.gt.md, 'q-mb-md': $q.screen.md || $q.screen.lt.md, 'col-12': $q.screen.lt.md}" v-for="user in headWinners")
        .user-card

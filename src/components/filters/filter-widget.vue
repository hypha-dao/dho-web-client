...
widget(:title="$t('filters.filter-widget.filters')")
  ...
  .row.items-center.justify-between.q-py-sm(v-if="showViewSelector")
    ...
    .btn-container
      q-btn.q-mr-xxs(unelevated rounded padding="12px" size="sm" icon="fas fa-th-large" :color="view === 'card' ? 'primary' : 'internal-bg'" :text-color="view === 'card' ? 'white' : 'primary'" @click="view = 'card'")
      q-btn(unelevated rounded padding="12px" size="sm" icon="fas fa-list" :color="view === 'list' ? 'primary' : 'internal-bg'" :text-color="view === 'list' ? 'white' : 'primary'" @click="view = 'list'")
  .row.items-center.justify-between.q-mt-sm(v-if="showToggle")
    .h-b2 {{ toggleLabel }}
    q-toggle(v-model="toggle" color="primary" keep-color="keep-color")
  template(v-if="$q.screen.lt.md")
    q-btn.q-my-sm.q-px-sm.full-width(:class="'btn-primary-active'" :label="$t('filters.filter-widget.saveFilters')" no-caps rounded unelevated @click="$emit('close-window')")
    q-btn.q-px-sm.full-width(:class="'internal-bg'" :label="$t('filters.filter-widget.resetFilters')" no-caps rounded unelevated @click="resetFilters")

...
export default {
    ...
    emojis () {
      if (this.search) {
        ...
        for (const category in emojiTable) {
          obj[category] = {}
          for (const emoji in emojiTable[category]) {
            if (new RegExp(`.*${escapeRegExp(this.search)}.*`).test(emoji)) {
              obj[category][emoji] = emojiTable[category][emoji]
            }
          }
  ...
  methods: {
    addEmoji (emoji) { this.$emit('emoji', emoji) },
    hidePanel () { this.isOpen = false },
    showPanel () { this.isOpen = true }
  }
...
<template lang="pug">
.emoji-picker.relative-position(v-click-outside="hidePanel")
  q-btn(@click="showPanel" unelevated flat)
    ...
    .emoji-picker__panel.bg-white.q-pa-sm(v-show="isOpen" )
      .row.full-width(
        ...
        :key='category'
        v-for='(emojiGroup, category, index) in emojis'
      )

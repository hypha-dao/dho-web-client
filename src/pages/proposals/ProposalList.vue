...
q-page.page-proposals
  ...
  .row.q-py-md(v-if="$q.screen.gt.md")
    .col-9
      base-placeholder.q-mr-sm(v-if="!proposals.length && !filteredStagedProposals.length && !filteredProposals.length && !stagedProposals.length" title= "No Proposals" subtitle="Your organization has not created any proposals yet. You can create a new proposal by clicking the button below."
      ...
      base-placeholder.q-mr-sm(v-if="!filteredProposals.length && !filteredStagedProposals.length && (proposals.length ||stagedProposals.length)" title= "Oops, nothing could be found here" subtitle="Try a different filter or another keyword"

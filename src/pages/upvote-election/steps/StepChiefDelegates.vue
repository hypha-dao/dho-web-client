...
.step-chief-delegates
    ...
    .row
      div
        .row
          ...
          .h-h5.text-bold.q-mx-sm {{ $t('pages.upvote-election.steps.stepchiefdelegates.voterBadge') }}
          .title-blue {{ $t('pages.upvote-election.steps.stepchiefdelegates.assigned') }}
          .h-h3.text-weight-600.q-ml-md {{ upvoteElectionData.votersBadgeCount }}
      q-separator(:style="{ 'margin': '-8px 50px', 'background': '#707070' }" vertical="vertical")
      div
        .row
          q-icon.card-icon(name="far fa-address-card" size="16px" color="white")
          .h-h5.text-bold.q-mx-sm {{ $t('pages.upvote-election.steps.stepchiefdelegates.memberInThisRound') }}
          .h-h3.text-weight-600.q-ml-md {{ upvoteElectionData.delegatesBadgeCount }}
  .row.no-wrap.items-center
    .h-h6.text-no-wrap {{ $t('pages.upvote-election.steps.stepchiefdelegates.totalVoters') }}
  ...
  .h-h6 {{ $t('pages.upvote-election.steps.stepchiefdelegates.eligibleForChief') }}
  .row.q-mt-md

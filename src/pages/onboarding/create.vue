      ...
      section.row.items-stretch(v-if="isState(['CREATED'])")
        widget.full-width
          ...
          .row.justify-end.q-mt-md
            q-btn.q-px-xl(@click="onGoToDashboard" color="primary" label="Go to Dashboard" no-caps="no-caps" rounded="rounded" unelevated="unelevated")
          ...
          div(:style="{'height': 'auto'}" :class="{ 'col-6 q-pl-xs': $q.screen.gt.md, 'q-mt-md': $q.screen.lt.md || $q.screen.md }")
            widget
              .row.justify-between
                ...
                .row.full-width.justify-end
                  q-btn.q-px-xl(@click="state = 'ADDING_ADMINS'" color="primary" label="Create Launch Team" no-caps="no-caps" outline="outline" rounded="rounded" unelevated="unelevated")
        ...
        widget#step-identity.q-mb-md(v-if="$q.screen.gt.md ? pastSteps.includes('step-identity') : activeStep === 'IDENTITY'" :class="{ 'disabled': (activeStep !== 'IDENTITY') && $q.screen.gt.md, 'ghost-margin-bottom': pastSteps.slice(-1)[0] === 'step-identity' }")
          ...
          .row.full-width.justify-between.q-mt-xl
            .col-7(:class="{ 'full-width': !$q.screen.gt.md, 'q-pr-md': $q.screen.gt.md }")
              label.h-label Name
              q-input.q-mt-xs.rounded-border(:rules="[rules.required, rules.min(3)]" dense="dense" lazy-rules="ondemand" maxlength="50" outlined="outlined" placeholder="The display name of your DAO (max. 50 character)" ref="title" v-model="form.title")
            .col-5(:class="{ 'full-width q-pt-md': !$q.screen.gt.md }")
              .row.justify-center.items-center
                ...
                .col.q-ml-md
                  ...
                  q-btn.full-width.rounded-border.text-bold.q-mt-xs(:class="{ 'q-px-xl': $q.screen.gt.md }" @click="$refs.ipfsInput.chooseFile()" color="primary" label="Upload an image" no-caps="no-caps" outline="outline" rounded="rounded" unelevated="unelevated")
            ...
            .col-12.q-mt-md(:class="{ 'full-width': !$q.screen.gt.md }")
              ...
              q-input.q-mt-xs.rounded-border(:input-style="{ 'resize': 'none' }" :rules="[rules.required]" dense="dense" lazy-rules="ondemand" maxlength="300" outlined="outlined" placeholder="Briefly explain what your DAO is all about (max. 300 characters)" ref="description" rows="10" type="textarea" v-model="form.description")
          nav.row.justify-end.q-mt-xl.q-gutter-xs
            q-btn.q-px-xl(@click="onNextStep(activeStepIndex)" color="primary" :label="'Next step'" no-caps="no-caps" rounded="rounded" unelevated="unelevated" v-if="$q.screen.gt.md")
        widget#step-token.q-mb-md(:class="{ 'disabled': (activeStep !== 'TOKEN') && $q.screen.gt.md, 'ghost-margin-bottom': pastSteps.slice(-1)[0] === 'step-token' }" v-if="$q.screen.gt.md ? pastSteps.includes('step-token') : activeStep === 'TOKEN'")
          ...
          .row.full-width.justify-between.q-mt-xl
            .col-8(:class="{ 'full-width': !$q.screen.gt.md, 'q-pr-md': $q.screen.gt.md }")
              label.h-label Name
                q-input.q-mt-xs.rounded-border(:debounce="200" :rules="[rules.required, rules.min(1), rules.max(20)]" bg-color="white" dense="dense" lazy-rules="ondemand" maxlength="20" outlined="outlined" placeholder="Max 20 characters. ex. Bitcoin" ref="utilityName" rounded="rounded" v-model="form.utilityName")
            .col-4(:class="{ 'full-width': !$q.screen.gt.md, '': $q.screen.gt.md }")
              label.h-label Symbol
              q-input.q-mt-xs.rounded-border(:debounce="200" :rules="[rules.required, rules.isTokenAvailable]" bg-color="white" dense="dense" lazy-rules="ondemand" mask="AAAAAAAA" maxlength="7" outlined="outlined" placeholder="Max 7 characters ex. BTC" ref="utilitySymbol" rounded="rounded" v-model="form.utilitySymbol")
          .row.q-mt-xl
            q-avatar.q-mr-xs(size="30px")
          ...
          .row.full-width.justify-between.q-mt-xl
            .col-8(:class="{ 'full-width': !$q.screen.gt.md, 'q-pr-md': $q.screen.gt.md }")
              label.h-label Name
                q-input.q-mt-xs.rounded-border(:debounce="200" :rules="[rules.required, rules.min(1), rules.max(20)]" bg-color="white" dense="dense" lazy-rules="ondemand" maxlength="20" outlined="outlined" placeholder="Max 20 characters. ex. Bitcoin" ref="utilityName" rounded="rounded" v-model="form.treasuryName")
            .col-4(:class="{ 'full-width': !$q.screen.gt.md, '': $q.screen.gt.md }")
              label.h-label Symbol
              q-input.q-mt-xs.rounded-border(:debounce="200" :rules="[rules.required, rules.isTokenAvailable]" bg-color="white" dense="dense" lazy-rules="ondemand" mask="AAAAAAAA" maxlength="7" outlined="outlined" placeholder="Max 7 characters ex. BTC" ref="treasurySymbol" rounded="rounded" v-model="form.treasurySymbol")
          nav.row.justify-end.q-mt-xl.q-gutter-xs
          ...
          .row.items-center.q-col-gutter-md.q-mt-xl
            ...
            .col-12.row
              ...
              .row(:class="{ 'full-width': !$q.screen.gt.sm, 'col': $q.screen.gt.sm }")
                ...
                .col-4(:class="{ 'full-width q-mt-md': !$q.screen.gt.sm, 'q-pr-md': $q.screen.gt.sm }")
                  q-btn.q-px-xl.full-width(:style="{'background': form.secondaryColor, 'color': form.textColor }" label="Secondary color" no-caps="no-caps" rounded="rounded" unelevated="unelevated")
        ...
        .row.full-width.justify-between.q-mt-xl
          .full-width(v-if="isState(['ADDING_ADMINS'])")
            ...
            nav.row.full-width.justify-end.q-mt-xs
              q-btn.text-bold.q-pa-none.q-ml-lg.q-mr-xs(flat="flat" color="primary" no-caps="no-caps" padding="none" @click="addTeamMember") Add to the team +
                ...
                .q-pa-sm.rounded-border.row.items-center.relative.member-item(:style="{'border': '1px solid var(--q-color-primary)'}")
                  ...
                  div
                    ...
                    p.q-pa-none.q-ma-none.font-sans.text-xs.text-weight-500.text-h-gray(v-if="member.fullName") @
                      | {{member.username}}
        nav.row.justify-end.q-mt-xl.q-gutter-xs
          q-btn.q-px-xl(@click="onGoToDashboard" color="primary" label="Go to Dashboard" no-caps="no-caps" outline="outline" rounded="rounded" unelevated="unelevated")

  ...
  q-dialog(:value="transactionReviewOpen" :full-width="successfullMultisigTransaction === null" @hide="transactionReviewOpen = false, successfullMultisigTransaction = null")
    ...
    widget.q-mt-md.full-width(:title="$t('pages.dho.treasury.transactionReview')")
      div(v-if="successfullMultisigTransaction === true")
        ...
        .row.q-mt-sm.q-pb-md
          .text-grey.h-b2 {{ $t('pages.dho.treasury.youCanNowFindTheMultisig') }}
          .text-bold.h-b2 {{ $t('pages.dho.treasury.other2TreasurersNeeds') }}
          .text-grey.h-b2.q-ml-xxs {{ $t('pages.dho.treasury.theTheTransactionIsReady') }}
            ...
            template(v-slot:body="props")
              q-tr.q-tr--no-hover(:props="props")
                ...
                q-td(key="amountEndorsed" :props="props")
                  span(v-if="props.row.amountPaid === 0") {{ $t('pages.dho.treasury.open') }}
                  span(v-if="props.row.amountPaid > 0 && props.row.amountPaid < getAmount(props.row.amountRequested)") {{ $t('pages.dho.treasury.pending') }}
                ...
                q-td(key="attestations" :props="props")
                  q-img.treasurer.q-mr-xs(v-if="props.row.paidBy && props.row.paidBy.details_creator_n && profiles[props.row.paidBy.details_creator_n]" :src="profiles[props.row.paidBy.details_creator_n].avatar" size="25px" :style="{ 'border-radius': '50%', 'height': '25px', 'width': '25px' }")
                    q-tooltip {{ $t('pages.dho.treasury.signedBy') }}
                      | {{ props.row.paidBy.details_creator_n }}
                  q-icon.icon-placeholder.q-mr-xs(v-if="props.row.paidBy && (!props.row.paidBy.details_creator_n || !profiles[props.row.paidBy.details_creator_n])" :key="`treasurer1_rd_${props.row.redemption_id}`" name="fas fa-user-circle" size="sm" color="white")
                    q-tooltip {{ $t('pages.dho.treasury.signedBy') }}
                      | {{ props.row.paidBy.details_creator_n }}
            ...
            .row.full-width.q-py-md.q-px-xl
              ...
              .col.flex.column
                ...
                .flex.justify-end.text-italic
                  span(v-if="item.amountPaid === 0") {{ $t('pages.dho.treasury.open') }}
